...
class PX4VisionEnv(gym.Env):
    """PX4 Gym Environment with Gazebo camera integration - FINAL VERSION"""
    ...
    def __init__(self, waypoints, max_steps=500, use_vision=False):
        ...
        # Action scaling - REDUCED for more stable learning
        self.max_roll_rate = 60.0    # Reduced from 100
        self.max_pitch_rate = 60.0   # Reduced from 100
        self.max_yaw_rate = 30.0     # Reduced from 45
        self.min_thrust = 0.50       # FIXED: Better balanced
        self.max_thrust = 0.80
        
        # Reward parameters
        self.waypoint_threshold = 1.5
        self.collision_threshold = 0.8  # INCREASED for more forgiveness
    ...
    def _stop_telemetry_tasks(self):
        """Cancels background tasks to prevent errors during restart."""
    ...
    async def _reset_drone(self):
        ...
        try:
            ...
            # FIXED: Wait for higher, more stable altitude
            safe_altitude = 3.5  # Increased from 2.5m
            start_wait = time.time()
            while True:
                ...
                if time.time() - start_wait > 20:
                    print("⚠️ Takeoff timed out, continuing anyway...")
                    break
                
                await asyncio.sleep(0.5)
            
            # CRITICAL: Stabilization wait
            print("Stabilizing drone (5 seconds)...")
            await asyncio.sleep(5)
            
            # Check stability
            vel_magnitude = np.linalg.norm(self.linear_velocity)
            final_alt = -self.position[2]
            print(f"Post-stabilization: Alt={final_alt:.2f}m, Vel={vel_magnitude:.2f}m/s")
            
            if vel_magnitude > 1.0:
                print("⚠️ High velocity detected, waiting longer...")
                await asyncio.sleep(3)
    ...
    def step(self, action):
        ...
        # Straitjacket boost - enforce stable climb for first 15 steps
    ...
    def _get_observation(self):
        """FIXED: Use altitude differences for intuitive observation space"""
        target_waypoint = self.waypoints[self.current_waypoint_idx]
        
        # Convert NED to altitude (down is negative, altitude is positive)
        drone_altitude = -self.position[2]
        waypoint_altitude = -target_waypoint[2]
        
        # Relative position with INTUITIVE altitude difference
        relative_position = np.array([
            target_waypoint[0] - self.position[0],  # North difference
            target_waypoint[1] - self.position[1],  # East difference
            waypoint_altitude - drone_altitude      # Altitude difference (positive = need to climb)
        ])
    ...
    def _calculate_reward(self):
        """FIXED: Proper altitude reward gradient"""
        target_waypoint = self.waypoints[self.current_waypoint_idx]
        ...
        # FIXED: Altitude rewards - STRONGLY encourage 5-10m altitude
        if altitude < 1.5:
            reward -= 50.0  # Critical danger zone
        elif altitude < 3.0:
            reward -= 10.0  # Too low
        elif altitude < 4.0:
            reward -= 3.0   # Still low
        elif 5.0 <= altitude <= 10.0:
            reward += 2.0   # PERFECT altitude - big reward!
        elif 10.0 < altitude <= 12.0:
            reward += 0.5   # Okay altitude
        elif altitude > 15.0:
            reward -= 3.0   # Getting too high
        elif altitude > 20.0:
            reward -= 10.0  # Way too high
